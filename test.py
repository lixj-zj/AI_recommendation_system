...
class baidu_nlp:
    ...
    def cifa(self):
        cifa = self.client.lexerCustom(self.text)
        items = cifa.items()
        # print(items[items])

    def lableOfArticles(self):
        print(self.text)
        lable = self.client.keyword(self.title, self.text)
        print(lable)
        return lable
...
if __name__ == '__main__':
    ...
    # baidu_nlp = baidu_nlp(title, text.encode('gbk'))
